...
export default function WeekNav({ weekOffset }: { weekOffset: number }) {
  ...
  return (
    <div className="flex items-center gap-1">
      <button
        onClick={() => router.push(`/schedule?week=${weekOffset - 1}&dir=prev`)}
      ...
      {weekOffset !== 0 && (
        <button
          onClick={() => router.push(`/schedule?dir=${weekOffset > 0 ? 'prev' : 'next'}`)}
      ...
      <button
        onClick={() => router.push(`/schedule?week=${weekOffset + 1}&dir=next`)}
        className="w-11 h-11 flex items-center justify-center rounded-xl text-label-2
          bg-[#e6dfd0] hover:bg-[#ddd4be] active:bg-[#cdbfa0] transition-colors duration-150"
