  ...
  return (
    <div className="max-w-2xl mx-auto px-6 pb-12 pt-page">
      <div className="flex items-center justify-between mb-10">
        <h1 className="text-3xl font-semibold tracking-tight text-label-1">People</h1>
        <NewEmployeeButton />
      </div>

      <div className="rounded-xl border border-[#d3c9b2] overflow-hidden [box-shadow:var(--shadow-sm)] stagger">
        {(employees ?? []).map(emp => {
              ...
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="relative flex-shrink-0">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                    isClockedIn ? 'bg-[#eef4f1]' : 'bg-[#eae3d3]'
                  }`}>
                    <span className={`text-xs font-semibold ${isClockedIn ? 'text-[#3d6b55]' : 'text-label-3'}`}>
                      {initials}
                    </span>
                  </div>
                  {isClockedIn && (
                    <div className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full bg-[#4a7c59] border-2 border-[#f9f4ea]" />
                  )}
                </div>
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-label-3 mt-0.5 capitalize tracking-[-0.01em]">{emp.role}</p>
                </div>
              </div>

              <svg
                className="w-4 h-4 text-label-3 group-hover:text-label-2 transition-colors duration-150 flex-shrink-0"
        ...
        {(employees ?? []).length === 0 && (
          <div className="px-4 py-8 text-center">
            <p className="text-sm text-label-3 tracking-[-0.01em]">No employees yet.</p>
          </div>
        )}
      </div>
    </div>
